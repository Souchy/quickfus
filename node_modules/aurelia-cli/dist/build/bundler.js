...
exports.Bundler = class {
    ...
    static create(project, packageAnalyzer, packageInstaller) {
        ...
        return Promise.all(project.build.bundles.map(x => Bundle.create(bundler, x).then(bundle => {
    ...
    build(opts) {
        ...
        const doTranform = (initSet) => {
            ...
            if (deps.size) {
                ...
                return Utils.runSequentially(Array.from(deps).sort(), d => {
    ...
    write() {
        return Promise.all(this.bundles.map(bundle => bundle.write(this.project.build.targets[0])))
            ...
            for (let i = this.bundles.length; i--;) {
                yield this.bundles[i].writeBundlePathsToIndex(this.project.build.targets[0]);
            }
        }));
    }
