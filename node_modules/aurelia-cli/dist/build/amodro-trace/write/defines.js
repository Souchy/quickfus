var lang = require('../lib/lang'), parse = require('../lib/parse'), transform = require('../lib/transform'), falseProp = lang.falseProp, getOwn = lang.getOwn, makeJsArrayString = lang.makeJsArrayString;
...
function defines(options) {
    ...
    return function (context, moduleName, filePath, _contents) {
        var namedModule, config = context.config, packageName = context.pkgsMainMap[moduleName];
        if (packageName === 'moment') {
            ...
            _contents = _contents.replace(/\bdefine\((\w+)\)/, (match, factoryName) => `(function(){var m=${factoryName}();if(typeof moment === 'undefined'){window.moment=m;} define(function(){return m;})})()`);
        }
        ...
        let contents = toTransport(context, moduleName, filePath, _contents, onFound, options);
...
function exportsFn(_exports, wrapShim) {
    if (_exports) {
        if (wrapShim)
            return 'return root.' + _exports + ' = ' + _exports + ';';
        else
 ...
 */
function toTransport(context, moduleName, filePath, contents, onFound, options) {
    options = options || {};
    return transform.toTransport('', moduleName, filePath, contents, onFound, options);
}
;
