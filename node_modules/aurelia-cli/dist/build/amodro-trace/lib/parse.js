...
var define = function (ary, fn) {
    module.exports = fn.apply(undefined, (ary.map(function (id) { return require(id); })));
};
...
define(['esprima', './lang'], function (esprima, lang) {
    ...
    emptyScope = {}, mixin = lang.mixin, hasProp = lang.hasProp;
    ...
    function parse(moduleName, fileName, fileContents, options) {
        options = options || {};
        //Set up source input
        var i, moduleCall, depString, moduleDeps = [], result = '', moduleList = [], needsDefine = true, astRoot = esprima.parse(fileContents);
    ...
    parse.definesRequire = function (fileName, fileContents) {
        var foundDefine = false, foundDefineAmd = false;
    ...
    parse.getAnonDeps = function (fileName, fileContents) {
        var astRoot = typeof fileContents === 'string' ?
            esprima.parse(fileContents) : fileContents, defFunc = this.findAnonDefineFactory(astRoot);
    ...
    parse.findConfig = function (fileContents) {
        /*jslint evil: true */
        var jsConfig, foundConfig, stringData, foundRange, quote, quoteMatch, quoteRegExp = /(:\s|\[\s*)(['"])/, astRoot = esprima.parse(fileContents, {
            loc: true
        });
    ...
    parse.renameNamespace = function (fileContents, ns) {
        var lines, locs = [], astRoot = esprima.parse(fileContents, {
            loc: true
        });
        ...
        if (locs.length) {
            ...
            locs.forEach(function (loc) {
                ...
                lineIndex = loc.start.line - 1, line = lines[lineIndex];
    ...
    parse.findDependencies = function (fileName, fileContents, options) {
        // modified to accept parsed result for efficiency.
        var dependencies = [], astRoot;
    ...
    parse.usesCommonJs = function (fileName, fileContents) {
        var uses = null, assignsExports = false;
        traverse(esprima.parse(fileContents), function (node) {
            var type, 
    ...
    parse.parseNode = function (node, onMatch, fnExpScope) {
        var name, deps, cjsDeps, arg, factory, exp, refsDefine, bodyNode, args = node && node[argPropName], callName = parse.hasRequire(node), isUmd = false;
    ...
    parse.getLicenseComments = function (fileName, contents) {
        ...
        ast = esprima.parse(contents, {
            comment: true,
            range: true
        }), result = '', existsMap = {}, lineEnd = contents.indexOf('\r') === -1 ? '\n' : '\r\n';
        if (ast.comments) {
